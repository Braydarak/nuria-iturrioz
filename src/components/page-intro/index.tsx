...
export default function PageIntro() {
  const [stage, setStage] = useState<'blue' | 'collapse' | 'fading' | 'hidden'>('blue')

  useEffect(() => {
    const collapseDurationMs = 1200

    const tBlueEnd = setTimeout(() => setStage('collapse'), 2000)
    const tFadeStart = setTimeout(() => setStage('fading'), 2000 + collapseDurationMs)
    const tHidden = setTimeout(() => setStage('hidden'), 2000 + collapseDurationMs + 300)
    return () => { clearTimeout(tBlueEnd); clearTimeout(tFadeStart); clearTimeout(tHidden) }
  }, [])

  if (stage === 'hidden') return null

  const fadingClass = stage === 'fading' ? 'opacity-0 transition-opacity duration-300' : ''
    ...
    <div className={`fixed inset-0 z-40 pointer-events-none ${fadingClass}`}>
      <style>
        {`
          ...
          .door-single { left: 0; width: 100%; transform-origin: right; transform: scaleX(1); }
          ...
          .run-close { animation: doorClose 1.2s cubic-bezier(0.4, 0, 0.2, 1) forwards; }
      ...
      {/* Logo centrado en blanco con filter y fade-out al cerrar */}
       <div className={`absolute inset-0 z-50 flex items-center justify-center transition-opacity duration-500 ${stage === 'blue' ? 'opacity-100' : 'opacity-0'}`}>
         <img
